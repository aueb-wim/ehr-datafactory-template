...
BEGIN
	COPY new_table FROM '/tmp/harmonized_clinical_data_long_anon.csv' DELIMITER ',' CSV HEADER ; 
EXCEPTION
WHEN OTHERS 
        THEN raise notice 'No such file /tmp/harmonized_clinical_data_long_anon.csv';
END;
COPY (SELECT * FROM new_table) TO '/tmp/harmonized_clinical_data_long_anon.csv' WITH CSV DELIMITER ',' HEADER;
