create or replace function pivotfunction_max_fed() RETURNS void AS $BODY$
...
BEGIN
	COPY new_table FROM '/tmp/harmonized_clinical_data_max_anon.csv' DELIMITER ',' CSV HEADER ; 
EXCEPTION
WHEN OTHERS 
        THEN raise notice 'No such file /tmp/harmonized_clinical_data_max_anon.csv';
END;
COPY (SELECT * FROM new_table) TO '/tmp/harmonized_clinical_data_max_anon.csv' WITH CSV DELIMITER ',' HEADER;
EXECUTE FORMAT('DROP TABLE IF EXISTS ' || table_name);
END; $BODY$ language plpgsql;
select pivotfunction_max_fed();
