create or replace function pivotfunction_long() RETURNS void AS $BODY$
...
BEGIN
	COPY new_table FROM '/tmp/harmonized_clinical_data_long.csv' DELIMITER ',' CSV HEADER ; 
EXCEPTION
WHEN OTHERS 
        THEN raise notice 'No such file /tmp/harmonized_clinical_data_long.csv';
END;
COPY (SELECT * FROM new_table) TO '/tmp/harmonized_clinical_data_long.csv' WITH CSV DELIMITER ',' HEADER;
EXECUTE FORMAT('DROP TABLE IF EXISTS temp_table');
EXECUTE FORMAT('DROP TABLE IF EXISTS ' || table_name);
END; $BODY$ language plpgsql;
select pivotfunction_long();
